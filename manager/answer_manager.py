...
#############################################################
# 大会サーバーのURLとトークン、試合時間を環境変数から取得
MATCH_URL = os.getenv("MATCH_SERVER_URL") or "http://localhost:3000"
MATCH_TOKEN = os.getenv("MATCH_SERVER_TOKEN") or "default_token"
MATCH_TIME = int(os.getenv("MATCH_SERVER_MATCH_TIME") or 300)

MANAGER_URL = os.getenv("MANAGER_SERVER_URL") or "http://localhost:5000"
CLIENT_ID_1 = os.getenv("MANAGER_CLIENT_ID_1") or "default_client_id"
CLIENT_ID_2 = os.getenv("MANAGER_CLIENT_ID_2") or "default_client_id"
CLIENT_ID_3 = os.getenv("MANAGER_CLIENT_ID_3") or "default_client_id"
CLIENT_ID_SPARE = os.getenv("MANAGER_CLIENT_ID_SPARE") or "default_client_id"
...
ip, port = re.match(r'http://(.*):(\d+)', MANAGER_URL).groups()
port = int(port)
tokens = [CLIENT_ID_1, CLIENT_ID_2, CLIENT_ID_3, CLIENT_ID_SPARE]
...
def send_answer(answer: str) -> tuple[int, int | None]:
    ...
    header = {'Content-Type': 'application/json', 'Procon-Token': MATCH_TOKEN}
    try:
        response = rq.post(MATCH_URL + '/answer', headers=header, data=answer)
